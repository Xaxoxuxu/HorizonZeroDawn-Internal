...
int APIENTRY WinMain(HINSTANCE hInst, HINSTANCE hInstPrev, PSTR cmdline, int cmdshow)
{
	...
	if (!procId)
	{
		MessageBox(NULL, std::string(procName + " not found!\n\nPress ok to exit.").c_str(), "Error", MB_OK);
		return EXIT_FAILURE;
	}
	...
	{
		MessageBox(NULL, "Failed opening a handle to process!\n\nPress ok to exit.", "Error", MB_OK);
		return EXIT_FAILURE;
	}
	...
	if (!loc)
	{
		MessageBox(NULL, "VirtualAllocEx failed!\n\nPress ok to exit.", "Error", MB_OK);
		return EXIT_FAILURE;
	}
	...
	if (!hThread)
	{
		MessageBox(NULL, "CreateRemoteThread failed!\n\nPress ok to exit.", "Error", MB_OK);
		return EXIT_FAILURE;
	}
